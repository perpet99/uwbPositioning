...
def draw_uwb_tag(x, y, txt, t):
    pos_x = -250 + int(x * boxinfo.meter2pixel)
    pos_y = 150 - int(y * boxinfo.meter2pixel)
    r = 20
...
def draw_uwb_anchor(frame, x, y,radius,text,color=(255, 0, 0)):
    ...
    if radius > 0 :
        frame = cv2.circle(frame, (tx, ty),int( radius*boxinfo.meter2pixel), (0, 255, 0), 5)
...
def main():
    ...
    layout = [[sg.Text('OpenCV Demo', size=(40, 1), justification='center', font='Helvetica 20')],
              ...
              [
                  sg.ReadButton('X+', size=(10, 1), font='Helvetica 14'),
                  sg.ReadButton('X-', size=(10, 1), font='Helvetica 14'),
                  sg.ReadButton('Y+', size=(10, 1), font='Helvetica 14'),
                  sg.ReadButton('Y-', size=(10, 1), font='Helvetica 14'),
              ],
              [
                  sg.ReadButton('CX+', size=(10, 1), font='Helvetica 14'),
                  sg.ReadButton('CX-', size=(10, 1), font='Helvetica 14'),
                  sg.ReadButton('CY+', size=(10, 1), font='Helvetica 14'),
                  sg.ReadButton('CY-', size=(10, 1), font='Helvetica 14'),
              ],
              [
                  sg.ReadButton('R+', size=(10, 1), font='Helvetica 14'),
                  sg.ReadButton('R-', size=(10, 1), font='Helvetica 14'),
              ],
    ...
    while True:
        ...
        if event == 'Exit' or values is None:
            ...
            boxinfo.changeSize(10,0)
        elif event == "X-":
            boxinfo.changeSize(-10,0)
        elif event == "Y+":
            boxinfo.changeSize(0,10)
        elif event == "Y-":
            boxinfo.changeSize(0,-10)
        elif event == "CX+":
            boxinfo.changeCenter(10,0)
        elif event == "CX-":
            boxinfo.changeCenter(-10,0)
        elif event == "CY+":
            boxinfo.changeCenter(0,10)
        elif event == "CY-":
            boxinfo.changeCenter(0,-10)
        elif event == "R+":
            boxinfo.changeRate(10)
        elif event == "R-":
            boxinfo.changeRate(-10)
        ...
        ret, frame = cap.read()
        if boxinfo.x == 0:
            boxinfo.y ,boxinfo.x = frame.shape[0:2]
        # gray = cv.cvtColor(frame, cv.COLOR_BGR2GRAY)

        # let img be the PIL image
        # img = Image.fromarray(frame)  # create PIL image from frame
        # frame = cv2.line(frame, (0, 0), (511, 511), (255, 0, 0), 5)
        mtrx = np.float32([[1, 0, boxinfo.cx],
                           [0, 1, boxinfo.cy]])

        frame = cv2.warpAffine(frame,mtrx,dsize=(boxinfo.x, boxinfo.y))
        # frame = cv2.resize(frame, dsize=(1280, 480), interpolation=cv2.WARP_FILL_OUTLIERS)
